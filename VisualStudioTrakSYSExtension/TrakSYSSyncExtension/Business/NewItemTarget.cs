...
namespace TrakSYSSyncExtension.Business
{
    ...
    {
        public static NewItemTarget Create(DTE2 dte, string trakSYSScriptType, string groupName)
        {
            ...
            if (item == null)
            {
                item = CreateFromSolutionExplorerSelection(dte, trakSYSScriptType, groupName);
            }
        ...
        private static NewItemTarget CreateFromSolutionExplorerSelection(DTE2 dte, string trakSYSScriptType, string groupName)
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            ...
            if (items.Length == 1)
            {
                ...
                else if (selection.Object is Project project)
                {
                    ...
                    {
                        string solutionFolder = GetSolutionFolderPath(project);
                        solutionFolder = Path.Combine(solutionFolder, trakSYSScriptType, groupName);
                        return new NewItemTarget(solutionFolder, project, null, isSolutionOrSolutionFolder: false);
                    }
                }
